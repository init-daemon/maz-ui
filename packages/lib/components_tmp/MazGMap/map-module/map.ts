...
export class Map {
  ...
  private getInfoWindow(options: MarkerInfoWindowOptions): google.maps.InfoWindow {
    if (options.offset)
      options.pixelOffset = new this.api.Size(options.offset.x, options.offset.y)
  ...
  public addMarker({ infoWindow, ...arguments_ }: MapMarker): google.maps.Marker {
  ...
  public addHTMLMarker({ position, templateHtml, offset, infoWindow }: MapHTMLMarker): HTMLMarker {
